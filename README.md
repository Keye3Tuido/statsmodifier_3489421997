...
- [新增指令到配置 UI：完整流程（手写版）](NEW_COMMAND_UI_GUIDE.md)
- UI 配置约束：不再提供模板函数，所有命令需手写 `col2` / `col3` 条目

## 游戏内命令界面 (Command UI)

由于官方调试控制台的指令回调（`MC_EXECUTE_CMD`）在当前版本中已失效，本模组提供了一个**游戏内 GUI 命令界面**作为替代方案。界面采用类似 macOS 访达的三列分栏设计，全程通过点击选择操作，无需记忆指令格式。

### 打开/关闭

- 按 **`\`** 键（反斜杠）或 **`|`** 键（Shift+反斜杠）打开/关闭命令界面
- 打开界面后所有游戏操作输入被拦截，关闭后自动恢复

### 界面布局

```
┌──────────────────────────────────────────────────┐
│ [属性] [指令]                      PlayerID:0    │  ← 顶部栏
├──────────────┬──────────────────┬────────────────┤
│ 移速         │ SET(=)           │ 参数: 5_       │
│ 射速         │ PLUS(+)          │                │
│ 伤害         │ TIMES(*)         │ 状态:          │
...
│ 全部清除     │                  │                │
├──────────────┴──────────────────┴────────────────┤
│ [\]开关  [Tab]切换  [Enter]执行  右键拖拽滚动    │
└──────────────────────────────────────────────────┘
  第一列：指令      第二列：选项         第三列：输入
```
...
| 列 | 内容 | 操作 |
|----|------|------|
| **第一列** | 指令列表（属性/命令），按功能分组 | 鼠标点击选择，选中项蓝色高亮 |
| **第二列** | 操作选项 | 属性修改：操作符（SET/PLUS/TIMES）+ 强制模式；可列举指令：选项列表；文本指令：输入提示 |
| **第三列** | 参数输入 + 状态显示 | 键盘输入参数值；实时显示当前状态 |

### 操作方式

1. **选择类别**：点击顶部 `[属性]` 或 `[指令]` 标签，或按 **Tab** 切换
2. **选择指令**：在第一列点击要执行的指令
3. **选择选项**（第二列）：点击操作符、勾选修饰符、或从列表中选择参数
4. **输入参数**（第三列）：键盘输入数值或文本
5. **执行**：按 **Enter** 执行指令
6. **玩家选择**：点击右上角 `PlayerID:X`，输入目标玩家 ID 后按 Enter 确认（自动校验）
7. **滚动列表**：↑/↓ 方向键滚动，或**右键按住拖拽**模拟滚动条
8. **查看帮助**：选中指令后点击 `[?]` 按钮查看详细说明

### 使用示例

| 操作 | 步骤 |
|------|------|
| 设置移速为 5 | 属性 → 移速 → SET(=) → 输入 `5` → Enter |
| 伤害翻倍 | 属性 → 伤害 → TIMES(*) → 输入 `2` → Enter |
| 强制设置幸运为 10 | 属性 → 幸运 → SET(=) → 勾选强制模式 → 输入 `10` → Enter |
| 切换蒙眼 | 指令 → 蒙眼 → Enter |
| 给予道具 | 指令 → 给予道具 → 输入道具 ID 或名称 → Enter |
| 传送房间 | 指令 → 传送 → 从列表选择或输入房间 ID → Enter |
| 切换挑战 | 指令 → 挑战标签 → 从列表选择挑战 → Enter |
...
切换安全包装状态。包装所有已注册的回调函数，使回调函数报错时不会导致游戏崩溃。
开启后自动拦截新注册的回调并包装。对Repentogon不生效。可能影响游戏性能。
指令格式：`wrap`
